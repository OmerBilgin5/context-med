...
program
  .name('context-sites')
  .description('Static site builder from wiki')
  .version(pkg.version);
...
program
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-sites] build --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      ...
      const result = { status: 'ok', package: 'context-sites', command: 'build' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-sites] build: completed');
      }
    }
  });
...
program
  .command('lint')
  .description('Lint wiki structure')
  .requiredOption('-i, --input <path>', 'Input file or directory')
  .option('-o, --output <path>', 'Output file or directory')
  .option('-f, --format <type>', 'Output format', 'json')
  .option('--dry-run', 'Simulate without writing files')
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-sites] lint --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      const outDir = path.dirname(path.resolve(opts.output));
      ...
      const result = { status: 'ok', package: 'context-sites', command: 'lint' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-sites] lint: completed');
      }
    }
  });

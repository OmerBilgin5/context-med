...
program
  ...
  .description('Qualitative analysis tool')
  .version(pkg.version);
...
program
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-qualitative] analyze --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      ...
      const result = { status: 'ok', package: 'context-qualitative', command: 'analyze' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-qualitative] analyze: completed');
      }
    }
  });
...
program
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-qualitative] compile --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      const outDir = path.dirname(path.resolve(opts.output));
      ...
      const result = { status: 'ok', package: 'context-qualitative', command: 'compile' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-qualitative] compile: completed');
      }
    }
  });

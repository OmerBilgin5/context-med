...
program
  .name('context-sim')
  .description('Clinical simulation compiler')
  .version(pkg.version);

// ─── compile ───────────────────────────────────────────────────────
program
  .command('compile')
  .description('Compile simulation scenario')
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-sim] compile --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      ...
      const result = { status: 'ok', package: 'context-sim', command: 'compile' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-sim] compile: completed');
      }
    }
  });
...
program
  .command('batch')
  .description('Batch compile scenarios')
  .requiredOption('-i, --input <path>', 'Input file or directory')
  .option('-o, --output <path>', 'Output file or directory')
  .option('-f, --format <type>', 'Output format', 'json')
  .option('--dry-run', 'Simulate without writing files')
  ...
  .action((opts) => {
    ...
    if (opts.dryRun) {
      console.log('[context-sim] batch --dry-run: validated, no output written.');
      return;
    }
    if (opts.output) {
      const outDir = path.dirname(path.resolve(opts.output));
      ...
      const result = { status: 'ok', package: 'context-sim', command: 'batch' };
      if (opts.format === 'json') {
        process.stdout.write(JSON.stringify(result, null, 2));
      } else {
        console.log('[context-sim] batch: completed');
      }
    }
  });
